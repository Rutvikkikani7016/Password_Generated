...
const PasswordGenerator = () => {
    ...
    const { passwordLength, includeUppercase, includeLowercase, password } = useSelector((state) => state);
                    ...
                    <div className="card">
                        <div className="card-body">
                            <form>
                                ...
                                <div className='row'>
                                    <div className="mb-3 col-3">
                                        <input type="number" className="form-control" min={1} value={passwordLength} onChange={(e) => dispatch(setPasswordLength(parseInt(e.target.value)))} />
                                    </div>
                                    <div className="mb-1 col-6">
                                        <input type="range" className="form-range" value={passwordLength}
                                            onChange={handleSliderChange} min={0} max={50} id="customRange" />
                                    </div>
                                </div>

                                <div className='row'>
                                    <label>
                                        <input type="checkbox" checked={includeUppercase} onChange={(e) => dispatch(setIncludeUppercase(e.target.checked))} />{' '} Include Uppercase
                                    </label>
                                    <label>
                                        <input type="checkbox" checked={includeLowercase} onChange={(e) => dispatch(setIncludeLowercase(e.target.checked))} />{' '} Include Lowercase
                                    </label>
                                </div>
